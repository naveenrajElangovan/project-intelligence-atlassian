...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    @app.get("/v1/health")
    async def health():
        return {"status": "ok", "environment": configured.environment, "accessMode": configured.access_mode}
    ...
    async def freshness():
        ...
        return {
            ...
            "lastEventCompletedAt": datetime.fromtimestamp(value, UTC).isoformat() if value else None,
    ...
    async def configure_callback(body: CallbackRequest, x_internal_api_key: Annotated[str | None, Header()] = None):
        ...
        if not configured.forge_config_url or not configured.forge_config_secret:
            raise HTTPException(status.HTTP_503_SERVICE_UNAVAILABLE, "Forge configuration endpoint is not configured.")
    ...
    @app.post("/v1/events/atlassian", response_model=EventAcceptance, response_model_by_alias=True, status_code=202)
    async def event(request: Request, x_atlassian_signature_256: Annotated[str | None, Header()] = None, x_atlassian_timestamp: Annotated[str | None, Header()] = None):
        ...
        verify_forge_signature(body, x_atlassian_signature_256, x_atlassian_timestamp, configured.forge_webhook_secret, configured.event_max_age_seconds)
        ...
        if parsed.self_generated:
            ...
            return EventAcceptance(accepted=False, reason="Self-generated event ignored.", eventId=parsed.event_id)
        accepted = await coordinator.accept(parsed)
        EVENTS.labels("accepted" if accepted else "duplicate", parsed.resource_type.value).inc()
        return EventAcceptance(accepted=accepted, duplicate=not accepted, reason="Accepted." if accepted else "Duplicate event.", eventId=parsed.event_id)

    @app.post("/v1/internal/search", response_model=McpResult)
    async def search(body: SearchRequest, x_internal_api_key: Annotated[str | None, Header()] = None):
        authorize_internal(x_internal_api_key, configured.internal_api_key)
        policy.authorize(body.tool)
        if body.identity_class == "USER":
            if not body.user_id:
                raise HTTPException(status.HTTP_422_UNPROCESSABLE_CONTENT, "A user identity is required for a user-scoped MCP read.")
            ...
            if session is None or (session.expires_at and session.expires_at <= datetime.now(UTC)):
                raise HTTPException(status.HTTP_401_UNAUTHORIZED, "No active user-scoped Atlassian MCP session is available.")
            authorization = f"Bearer {session.access_token}"
            credential_fingerprint = session.access_token
        else:
            if body.user_id:
                raise HTTPException(status.HTTP_422_UNPROCESSABLE_CONTENT, "A service-identity read cannot impersonate a user.")
        ...
        if not credential_fingerprint:
            raise HTTPException(status.HTTP_503_SERVICE_UNAVAILABLE, "The requested Atlassian MCP identity is not configured.")
        client_key = (
            f"user:{body.project_id}:{body.user_id}"
            if body.identity_class == "USER"
            else "service"
        )
        ...
        return McpResult(tool=body.tool, complete=bool(result.get("complete", False)), content=content)

    @app.put("/v1/internal/sessions")
    async def register_session(body: SessionRegistration, x_internal_api_key: Annotated[str | None, Header()] = None):
        authorize_internal(x_internal_api_key, configured.internal_api_key)
    ...
    async def delete_session(project_id: str, user_id: str, x_internal_api_key: Annotated[str | None, Header()] = None):
        authorize_internal(x_internal_api_key, configured.internal_api_key)
        user_sessions.pop((user_id, project_id), None)
        mcp_clients.pop(f"user:{project_id}:{user_id}", None)

    @app.post("/v1/internal/mutations")
    async def mutation(_: MutationRequest, x_internal_api_key: Annotated[str | None, Header()] = None):
        authorize_internal(x_internal_api_key, configured.internal_api_key)
        raise HTTPException(status.HTTP_403_FORBIDDEN, detail={"code": "ATLASSIAN_WRITE_DISABLED", "message": "Atlassian write access is disabled."})

    @app.post("/v1/internal/rest-read")
    async def rest_read(body: RestReadRequest, x_internal_api_key: Annotated[str | None, Header()] = None):
        authorize_internal(x_internal_api_key, configured.internal_api_key)
        headers = {"Accept": "*/*"}
        if configured.backend_internal_api_key:
            headers["Authorization"] = f"Bearer {configured.backend_internal_api_key}"
        query = [("target", body.target), *((key, str(value)) for key, value in body.params.items())]
