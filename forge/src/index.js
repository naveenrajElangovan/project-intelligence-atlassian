...
function resource(event) {
  ...
  const comment = event.comment || (String(event.eventType || "").endsWith(":comment") ? event.content || {} : {});
  const worklog = event.worklog || {};
  const attachment = event.attachment || {};
  const content = event.content || attachment.container || {};
  if (comment.id) return ["COMMENT", String(comment.id), String(issue.id || comment.container?.id || comment.content?.id || ""), String(issue.fields?.project?.key || comment.space?.id || comment.content?.space?.id || "")];
  if (worklog.id) return ["WORKLOG", String(worklog.id), String(issue.id || worklog.issueId || ""), String(issue.fields?.project?.key || "")];
  if (attachment.id) return ["ATTACHMENT", String(attachment.id), String(issue.id || attachment.container?.id || attachment.issueId || ""), String(issue.fields?.project?.key || attachment.space?.id || "")];
  if (issue.id) return ["ISSUE", String(issue.id), null, issue.fields?.project?.key || ""];
  return [content.type === "blogpost" ? "BLOGPOST" : "PAGE", String(content.id || ""), null, String(content.space?.id || "")];
}
...
function resources(event) {
  if (event.sourceIssueId && event.destinationIssueId) {
    ...
    return [
      ...
      ["ISSUE_LINK", linkId, String(event.destinationIssueId), String(event.destinationProjectId || "")],
    ];
  }
...
export async function run(event, context) {
  ...
  for (const [resourceType, resourceId, parentResourceId, projectOrSpaceId] of resources(event)) {
    ...
    const eventId = crypto.createHash("sha256").update(JSON.stringify([event.eventType, event.eventCreatedDate, resourceType, resourceId, parentResourceId])).digest("hex");
...
export async function configure(request) {
  ...
  const authorization = request.headers?.authorization?.[0] || request.headers?.Authorization?.[0] || "";
  if (!expected || authorization !== `Bearer ${expected}`) {
    return {statusCode: 401, headers: {"content-type": ["application/json"]}, body: JSON.stringify({error: "unauthorized"})};
  }
  const body = JSON.parse(request.body || "{}");
  if (typeof body.url !== "string" || !body.url.startsWith("https://")) {
    return {statusCode: 422, headers: {"content-type": ["application/json"]}, body: JSON.stringify({error: "invalid callback"})};
  }
